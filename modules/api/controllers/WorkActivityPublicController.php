			...
			if (!empty($activityId)) {
				...
				if (!empty($is_detail)) {
					...
					foreach ($replyData as $repl) {
						...
						array_push($reply, $repl);
					}
					Yii::error($reply, 'final');
			...
			if ($activity["type"] == 2) {
				...
				$sql              = "
						FROM
							...
							LEFT JOIN {{%work_external_contact_follow_user}} as g on (a.user_id = g.user_id and a.external_userid = g.external_userid)
...
//					$followUser = WorkExternalContactFollowUser::find()->alias("a")
//						->leftJoin("{{%work_user}} as b", "a.user_id = b.id")
//						->where(["a.external_userid" => $fan["external_userid"]])
//						->andWhere(["in", "a.user_id", explode(",", $fan["channel_user_id"])])
...
//						if ($item["del_type"] == 0) {
//							$fan["user_name"] .= $item["name"] . "（已添加）\r\n";
//						} else {
//							$fan["user_name"] .= $item["name"] . "（已删除）\r\n";
//						}
//					}
				}
			}
